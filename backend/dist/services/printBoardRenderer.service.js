...
exports.renderPrintBoardPdf = renderPrintBoardPdf;
const pdf_lib_1 = require("pdf-lib");
const printDimensions_1 = require("@utils/printDimensions");
const tileColorSwatches_1 = require("@utils/tileColorSwatches");
function drawWrappedText(page, text, x, y, maxWidth, font, size, color = (0, pdf_lib_1.rgb)(0.13, 0.12, 0.09)) {
...
function drawSwatch(page, tile, x, y, w, h, regularFont, boldFont) {
    const [r, g, b] = (0, tileColorSwatches_1.colorForTile)(tile);
    page.drawRectangle({ x, y: y - h, width: w, height: h, color: (0, pdf_lib_1.rgb)(r, g, b), borderColor: (0, pdf_lib_1.rgb)(0.13, 0.12, 0.09), borderWidth: 0.75 });
    const labelSize = Math.max(7, Math.min(11, w / 14));
    const labelY = y - h - labelSize - 4;
    page.drawText(tile.role.toUpperCase(), { x, y: labelY, size: labelSize * 0.8, font: boldFont, color: (0, pdf_lib_1.rgb)(0.68, 0.51, 0.31) });
    ...
    if (tile.brandName || tile.size) {
        ...
        page.drawText(meta, { x, y: labelY - labelSize * 3, size: labelSize * 0.8, font: regularFont, color: (0, pdf_lib_1.rgb)(0.47, 0.44, 0.36) });
    }
}
async function renderPrintBoardPdf(input) {
    const widthPt = (0, printDimensions_1.toPoints)(input.widthValue, input.unit);
    const heightPt = (0, printDimensions_1.toPoints)(input.heightValue, input.unit);
    ...
    page.drawText('CASA DE AURUM', { x: margin, y: cursorY - 14, size: 16, font: boldFont, color: (0, pdf_lib_1.rgb)(0.11, 0.1, 0.08) });
    page.drawText(input.boardName, { x: margin, y: cursorY - 32, size: 12, font: regularFont, color: (0, pdf_lib_1.rgb)(0.47, 0.44, 0.36) });
    cursorY -= 52;
    ...
    if (input.layout === 'HERO_IMAGE') {
        ...
        if (base)
            drawSwatch(page, base, margin, cursorY, contentWidth, heroHeight, regularFont, boldFont);
    ...
    page.drawLine({ start: { x: margin, y: footerY + 14 }, end: { x: widthPt - margin, y: footerY + 14 }, thickness: 0.5, color: (0, pdf_lib_1.rgb)(0.75, 0.71, 0.62) });
    drawWrappedText(page, `Grout: ${input.groutRecommendation || 'Not specified'}`, margin, footerY, contentWidth, regularFont, 9);
    drawWrappedText(page, `Brief: ${input.clientBrief}`, margin, footerY - 16, contentWidth, regularFont, 8, (0, pdf_lib_1.rgb)(0.47, 0.44, 0.36));
    ...
    page.drawText(specs, { x: margin, y: margin, size: 7, font: regularFont, color: (0, pdf_lib_1.rgb)(0.6, 0.57, 0.5) });
    return doc.save();
}
//# sourceMappingURL=printBoardRenderer.service.js.map